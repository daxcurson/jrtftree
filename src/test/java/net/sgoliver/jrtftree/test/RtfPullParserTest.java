...
public class RtfPullParserTest // In Sync
{
	...
	public void parseSimpleDocument() {
		...
		try {
			parser.loadRtfFile(getClass().getClassLoader().getResource("testdocs/testdoc1.rtf").getFile());
		} catch (FileNotFoundException e) {
			Assertions.fail("No se ha podido encontrar el archivo");
		} catch (IOException e) {
			Assertions.fail("Hay un error de lectura del archivo");
		}
	...
	public void parseSimpleRtfText() {
		...
		URL url = getClass().getClassLoader().getResource("testdocs/testdoc1.rtf");
		String archivo = url.getFile();
		tree.loadRtfFile(archivo);
